...
export default function NewCallPage() {
  ...
  return (
    <div className="mx-auto h-full max-w-lg overflow-y-auto px-6 py-6">
      <h1 className="text-xl font-bold text-sea-800">Log a Call</h1>
          ...
          <div className="mt-2 flex flex-col gap-1">
            {results.map((a) => (
              ...
              >
                ...
                {a.phone && (
                  <span className="ml-2 text-xs text-gray-400" dir="ltr">
                    {a.phone}
                  </span>
                )}
              </button>
            ))}
            {results.length === 0 && <p className="p-3 text-sm text-gray-400">No shops match.</p>}
          </div>
        </div>
      ) : (
